...
public class MrCrayfishGunCompatLMG {
    @SubscribeEvent
    ...
    public static void shooterBehaviour(Item ammoItem, ItemStackHandler itemStackHandler, Level level, Player player, Vec3 motion, double x, double y, double z) {
        if (ammoItem instanceof IAmmo) {
            ...
            LMGDummy = heavyRifleDummy;


            GunItem gunItem = (GunItem) LMGDummy.getItem();
            ...
            ProjectileEntity projectile = projectileFactory.create(level, player, LMGDummy, gunItem, gun);
            projectile.setWeapon(LMGDummy);
